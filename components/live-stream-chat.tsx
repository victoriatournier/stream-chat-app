    ...
    <div className="w-[480px] h-[600px] bg-gray-900 text-white flex flex-col rounded-lg overflow-hidden border border-gray-700">
      <div className="flex items-center justify-between p-2 bg-gray-800 border-b border-gray-700">
        ...
        <div className="flex items-center gap-1">
          <Button variant="ghost" size="sm" className="p-1 h-auto"><MoreVertical className="w-3 h-3 text-gray-400" /></Button>
          <Button variant="ghost" size="sm" className="p-1 h-auto"><X className="w-3 h-3 text-gray-400" /></Button>
        </div>
      </div>
      <div className="flex-1 p-1 flex flex-col justify-end overflow-hidden">
        <div className="space-y-1">
          {messages.slice(-20).map((message) => (
            <div key={message.id} className="flex items-start gap-1 group hover:bg-gray-800/30 p-0.5 rounded">
              <Avatar className="w-5 h-5 flex-shrink-0">
                <AvatarImage src={message.avatar || "/placeholder.svg"} />
                <AvatarFallback className="text-[10px] bg-gray-600">{message.username.charAt(0).toUpperCase()}</AvatarFallback>
              </Avatar>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-0.5 flex-wrap">
                  <span className="font-semibold text-xs truncate" style={{ color: message.usernameColor }}>{message.username}</span>
                  {message.isModerator && (<Badge className="bg-green-600 text-white text-[10px] px-1 py-0 h-4">MOD</Badge>)}
                  {message.isMember && (<Badge className="bg-purple-600 text-white text-[10px] px-1 py-0 h-4">#3</Badge>)}
                  {message.isSubscriber && (<Badge className="bg-blue-600 text-white text-[10px] px-1 py-0 h-4">SUB</Badge>)}
                </div>
                <p className="text-xs text-gray-100 break-words leading-tight mt-0.5">{message.message}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="flex items-center justify-between p-2 bg-gray-800 border-t border-gray-700">
        ...
        <div className="flex items-center gap-1">
          <Button variant="ghost" size="sm" className="p-1 h-auto"><span className="text-[10px] bg-gray-700 px-1 py-0.5 rounded">$</span></Button>
          <Button variant="ghost" size="sm" className="p-1 h-auto"><Heart className="w-3 h-3 text-gray-400" /></Button>
        </div>
      </div>
    </div>
  )
